				...
				try
					{
						...
						if ( $result )
							{
								return [ $this->finalResponse( 'success', 'Loan applicaton successful!', 'Your loan has been sent for approval, you will receive a confirmation post approval.',['Loan Status'=>'PENDING'] ) ];
							}
						else
							{
				...
				if ( $this->doesUserExist( $request->username, $request->secret ) )
					{
                        ...
                        $data =[
                            'Installment amount'=>$paymentsMeta->installment_amount,
						...
						if ( $result )
							{
								...
								if ( $loan_result )
									{
										return [ $this->finalResponse( 'success', 'Loan applicaton approved!', 'User can start paying EMI now. You can set an email ( Using mailgun or any other ) / SMS notification ( If DLT Registration done ) to inform the user for further process.', $data ) ];
									}
								else
									{
										$this->finalResponse();
						...
						if ( $meta_result )
							{
								if ( $meta_result->pending_amount <= 0 )
									{
										$this->finalResponse( 'success', 'EMIs Completed!', 'All EMIs were paid were you, no EMIs pending',['Loan Status'=>'PAID'] );
									}
